...
export async function handleVendaDocumentoArquivo(req: Request, res: Response) {
  ...
  const arquivo = await vendaDocumentosService.carregarArquivo(user.id, documentoId);
  if (!arquivo) {
    res.status(404).send("Documento não encontrado.");
    return;
  }
...
export function registerVendaDocumentosHttp(app: Application) {
  app.get("/api/vendas/documentos/:id/arquivo", handleVendaDocumentoArquivo);
}
