  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS \`compra_recebimentos\` (
        ...
        \`motivo_estorno\` varchar(80) NULL,
        \`observacao_estorno\` text NULL,
        \`estornado_por_user_id\` int NULL,
    ...
    `);
    if (
      (await tableExists(pool, "compra_recebimentos")) &&
      (await tableExists(pool, "compras")) &&
      !(await foreignKeyExists(pool, "compra_recebimentos", "compra_recebimentos_compra_fk"))
    ) {
      await pool.query(`
        ALTER TABLE \`compra_recebimentos\`
          ...
          ON UPDATE RESTRICT
      `);
      console.log("[schema] FK adicionada: compra_recebimentos.compra_recebimentos_compra_fk");
    }
    if (
      (await tableExists(pool, "compra_recebimentos")) &&
      (await tableExists(pool, "compra_grupos")) &&
      !(await foreignKeyExists(pool, "compra_recebimentos", "compra_recebimentos_grupo_fk"))
    ) {
      await pool.query(`
        ALTER TABLE \`compra_recebimentos\`
          ...
          ON UPDATE RESTRICT
      `);
      console.log("[schema] FK adicionada: compra_recebimentos.compra_recebimentos_grupo_fk");
    }

    if (await tableExists(pool, "pesagens")) {
      await ensureColumn(pool, "pesagens", "compraRecebimentoId", "int");
      if (!(await indexExists(pool, "pesagens", "pesagens_compra_recebimento_idx"))) {
        await pool.query(
          "CREATE INDEX `pesagens_compra_recebimento_idx` ON `pesagens` (`compraRecebimentoId`)",
        );
        console.log("[schema] Índice adicionado: pesagens.pesagens_compra_recebimento_idx");
      }
      if (
        (await tableExists(pool, "compra_recebimentos")) &&
        !(await foreignKeyExists(pool, "pesagens", "pesagens_compra_recebimento_fk"))
      ) {
        await pool.query(`
          ALTER TABLE \`pesagens\`
            ...
            ON UPDATE RESTRICT
        `);
        console.log("[schema] FK adicionada: pesagens.pesagens_compra_recebimento_fk");
      }
    }

    if (await tableExists(pool, "animal_lote_movimentacoes")) {
      await ensureColumn(pool, "animal_lote_movimentacoes", "compraRecebimentoId", "int");
      if (!(await indexExists(pool, "animal_lote_movimentacoes", "animal_lote_mov_compra_recebimento_idx"))) {
        await pool.query(
          "CREATE INDEX `animal_lote_mov_compra_recebimento_idx` ON `animal_lote_movimentacoes` (`compraRecebimentoId`)",
        );
        console.log("[schema] Índice adicionado: animal_lote_movimentacoes.animal_lote_mov_compra_recebimento_idx");
      }
      if (
        (await tableExists(pool, "compra_recebimentos")) &&
        !(await foreignKeyExists(pool, "animal_lote_movimentacoes", "animal_lote_mov_compra_recebimento_fk"))
      ) {
        await pool.query(`
          ALTER TABLE \`animal_lote_movimentacoes\`
            ...
            ON UPDATE RESTRICT
        `);
        console.log("[schema] FK adicionada: animal_lote_movimentacoes.animal_lote_mov_compra_recebimento_fk");
      }
    }

    if (await tableExists(pool, "compra_recebimentos")) {
      await ensureColumn(pool, "compra_recebimentos", "motivo_estorno", "varchar(80) NULL");
      await ensureColumn(pool, "compra_recebimentos", "observacao_estorno", "text NULL");
